...
class Settings(BaseSettings):
    ...
    # Кто выполняет дешёвый массовый слой — классификацию и сопоставление
    # с проектом: "groq" или "openai". Groq дешевле на порядок, но на
    # бесплатном тарифе упирается в лимит и разбирает очередь сутками.
    # Переключение на openai нужно, когда важнее скорость, чем цена.
    ANALYSIS_PROVIDER: str = "groq"
    # Модель для этого слоя. Пусто — берётся GROQ_MODEL или OPENAI_MODEL
    # в зависимости от провайдера. Для openai разумный выбор — gpt-4.1-mini:
    # на порядок дешевле gpt-4.1 и держит strict json_schema.
